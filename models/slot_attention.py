...
'''
For Diffrentiable Top-k Slot Selection
'''
...
def relaxed_topk(logits, k, temperature=1.0):
    ...
    logits: (B, S)
    returns:
        soft_k_hot: (B, S), approximately sums to k
    """
    scores = logits
    ...
    for _ in range(k):
        ...
        # suppress mass already assigned in this step
...
def parallel_topk_st(logits, k=1, temperature=1.0):
    ...
    forward:
        hard k-hot from Gumbel-TopK
    backward:
        dense soft surrogate scaled to sum to k
    """
    noised_logits = logits + gumbel_noise(logits)
